...
impl DffChunks {
	fn next<R>(&mut self, reader: &mut R) -> std::result::Result<bool, FileParseError>
	where
		R: Read,
	{
	...
	fn skip<R>(&self, reader: &mut R) -> std::result::Result<(), FileParseError>
	where
		R: Seek,
	{
...
fn verify_dff<R>(reader: &mut R) -> std::result::Result<(), FileParseError>
where
	R: Read,
{
	let mut id = [0; 16];
...
) -> std::result::Result<DffFile, FileParseError>
where
	R: Read + Seek,
{
	verify_dff(reader)?;
	...
	if sample_rate == 0 || channels == 0 {
		return Err(FileParseError::new(
			FileType::Dff,
			"Missing required FS or CHNL chunk".into(),
		));
	}
...
fn read_id3_chunk<R>(
	reader: &mut R,
	parse_options: ParseOptions,
) -> std::result::Result<crate::id3::v2::Id3v2Tag, FileParseError>
where
	R: Read + Seek,
{
...
fn parse_comt_chunk<R>(
	reader: &mut R,
	comt_size: u64,
) -> std::result::Result<super::DffTextChunks, FileParseError>
where
	R: Read + Seek,
{
